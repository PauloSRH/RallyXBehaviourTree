...
class Config:
	...
	# map_border[0] <= position[0] >= map_border[0] + map_size[0] and map_border[1] <= position[1] >= map_border[1] + map_size[1]
	...
	def collisions(self):
		...
		while participants:
			...
			for other in participants[1:]:
				needed_x_collision_difference = Config.object_size[0]
				needed_y_collision_difference = Config.object_size[1]
				diff_x = participant.position[0] - other.position[0]
				diff_y = participant.position[1] - other.position[1]
				if isinstance(participant, Computer) and isinstance(other, Player):
					needed_x_collision_difference = Config.object_size[0] - (int)(Config.object_size[0]/5)
					needed_y_collision_difference = Config.object_size[1] - (int)(Config.object_size[1]/5)
				if isinstance(other, Computer) and isinstance(participant, Player):
					needed_x_collision_difference = Config.object_size[0] - (int)(Config.object_size[0]/5)
					needed_y_collision_difference = Config.object_size[1] - (int)(Config.object_size[1]/5)
					
				if abs(diff_x) < needed_x_collision_difference and \
					abs(diff_y) < needed_y_collision_difference:
...
class Player(Car):
	...
	def __init__(self, *args):
		...
		self.path_to_flag = None
		self.desired_flag = None
		self.should_move_to_flag = True
		self.retries = 4
		self.behavior = self.run_behaviour_tree()
	...
	# Main method that makes the blue car performs his actions
	def update(self):
		self.behavior.run()
		self.update_fuel(1)
		...
		# Returns True if the given map position is a rock
	def is_rock(self, position):
		for rock in self.game.rocks:
			rock_map_position = exact_to_map(rock.position)
			if rock_map_position[0] == position[0] and rock_map_position[2] == position[1]:
				return True
		return False

	# Returns True if the blue car should send a smoke, False otherwise
	# If the nearest car is already smoked, return False
	# If there is a wall between the two cars, return False
	def should_send_smoke(self):
		if self.nearest_red_car.smoked:
			return False
		my_map_position = exact_to_map(self.position)
		nearest_car_map_position = exact_to_map(self.nearest_red_car.position)
		minX = min(my_map_position[0], nearest_car_map_position[0])
		maxX = max(my_map_position[0], nearest_car_map_position[0])
		minY = min(my_map_position[2], nearest_car_map_position[2])
		maxY = max(my_map_position[2], nearest_car_map_position[2])
		
		for i in range(minX, maxX + 1):
			if self.is_wall((i, my_map_position[2])) or self.is_wall((i, nearest_car_map_position[2])):
				return False
			
		for i in range(minY, maxY + 1):
			if self.is_wall((my_map_position[0], i)) or self.is_wall((nearest_car_map_position[0], i)):
				return False
			
		# Check if both cars are on the same Y to check if there is a rock on the same Y as them
		if my_map_position[2] == nearest_car_map_position[2]:
			minX = min(my_map_position[0], nearest_car_map_position[0])
			maxX = max(my_map_position[0], nearest_car_map_position[0])
			for i in range(minX + 1, maxX):
				if self.is_rock((i, my_map_position[2])):
					return False # Do not throw a smoke as there is a rock separating the cars
				
		# Check if both cars are on the same X to check if there is a rock on the same X as them
		if my_map_position[0] == nearest_car_map_position[0]:
			minY = min(my_map_position[2], nearest_car_map_position[2])
			maxY = max(my_map_position[2], nearest_car_map_position[2])
			for i in range(minY + 1, maxY):
				if self.is_rock((my_map_position[0], i)):
					return False # Do not throw a smoke as there is a rock separating the cars
		
		return True

	# The behaviour tree which will be executed
	def run_behaviour_tree(self):
		return Selector(
			Sequence(
				Atomic(self.calculate_nearest_car),
                Atomic(self.check_nearest_car_distance),
                Atomic(self.is_one_car_active),
				Atomic(self.clear_path),
				Atomic(self.send_smoke),
				Atomic(self.is_nearest_car_too_close),
				Atomic(self.avoid_car_helper)
					),
				Sequence(
					Selector(
						Sequence(Atomic(self.check_if_has_not_nearest_flag),
							Atomic(self.calculate_nearest_flag)),
						Atomic(self.succeed)
						),
					Selector(
						Sequence(Atomic(self.check_if_has_not_path_to_flag),
							Atomic(self.calculate_path)),
						Atomic(self.succeed)
						),
				Atomic(self.move_player)
				)
			)
			
	# Moves the player
	def move_player(self):
		# If we have a path to flag, then move to it
		if self.path_to_flag is not None and len(self.path_to_flag) > 0:
			self.move_to_flag()
		else:
			# If no path was found the player should stay on the same position
			self.requested_direction = (0,0)
		
		return True
	
	# Returns True if at least one car is active, False otherwise
	def is_one_car_active(self):
		for red_car in self.game.red_cars:
			if red_car.immobile == 0: # If one car is active
				return True
		return False
	
	# Clear the path to the flag and clear the nearest flag object
	def clear_path(self):
		self.path_to_flag = []
		self.nearest_flag = None
		return True
	
	# Returns True if the distance of the nearest red car is less than Config.object_size[0] * 2 
	def is_nearest_car_too_close(self):
		return self.distance(self.nearest_red_car) < Config.object_size[0] * 2
	
	# Calculates the object which has the shortest manhattan distance with the players position
	def get_manhattan_distances(self, objects):
		distances = []
		...
		for obj in objects:
			...
			temp_distance = abs(dx) + abs(dy)
			if temp_distance < min_distance:
				min_distance = temp_distance
				closest_object = obj
		if objects is not None:
			distances.append(closest_object)
		return distances

	# Calculates the eucledean distance between the position of the blue car and another position
	def distance(self, pos):
		dx = pos.position[0] - self.position[0]
		dy = pos.position[1] - self.position[1]
		return math.sqrt(dx * dx + dy * dy)
	
	# Calculates the flag which has the shortest eucledean distance to the position of the player
	def get_best_flag(self, flags):
		distances = []
		min_distance = float("inf")
		closest_object = None
		
		# When there is only one flag, always choose that one
		if len(flags) == 1:
			closest_object = flags[0]
			self.reset_retries()
		else:	
			for flag in flags:
				# If the number of tries for going to the flag in stuckPosition is lower than 1,
				# then we can not pick that flag and we choose the next one with the shortest,
				# eucledean distance to the position of the player
				if self.retries < 1 and (flag.position[0] == self.desired_flag.position[0] and flag.position[1] == self.desired_flag.position[1]):
					continue
				
				# If this flag is of type L, set it to the closest flag as we want to maximize the score
				if flag.flag_type is not None and flag.flag_type == "L":
					closest_object = flag
					break
				dx = flag.position[0] - self.position[0]
				dy = flag.position[1] - self.position[1]
				temp_distance = abs(dx) + abs(dy)
				if temp_distance < min_distance:
					min_distance = temp_distance
					closest_object = flag
		if flags is not None:
			...
			# Since the number of tries has been exceeded and we have chosen a different flag from the one where the number of retries were exceeded
			# Then reset the retries and set the desired_flag to none
			if self.retries < 1 and (self.desired_flag.position[0] != closest_object.position[0] and self.desired_flag.position[1] != closest_object.position[1]):
				self.reset_retries()
		return distances
	
	# Initializes data structures needed by A star algorithm
	def initialize_a_star(self,start, goal):
		g_dictionary = { start : 0 }
		closed_set = set()
		open_set = []
		heappush(open_set, (0, Node(0, self.heuristic(start, goal), start)))
		return open_set, g_dictionary, closed_set 
	
	# Gets the reversed path obtained from A star algorithm after reaching the goal
	def path(self, s1):
		path = []
		while s1.get_father() != None:
			path.append(s1.get_position())
			s1 = s1.get_father()
		path.append(s1.get_position())
		return path

	# Gets the neighbour positions of a cell in the map, used by A star algorithm
	def neighbours_of(self, atual):
		up = (atual[0], atual[1] - 1)
		down = (atual[0], atual[1] + 1)
		left = (atual[0] - 1, atual[1])
		right = (atual[0] + 1, atual[1])
		neighbours = []
		if self.is_valid_map_position(up):
			neighbours.append(up)

		if self.is_valid_map_position(down):
			neighbours.append(down)

		if self.is_valid_map_position(left):
			neighbours.append(left)

		if self.is_valid_map_position(right):
			neighbours.append(right)

		return neighbours

	# Calculates the manhattan distance between two positions
	def heuristic(self, nbr, pos):
		dx = pos[0] - nbr[0]
		dy = pos[1] - nbr[1]
		return abs(dx) + abs(dy)

	# Checks if the given position by A star algorithm is a valid map position
	# If the position has a rock in it return False
	# Else if the position has a car on the adjacent map positions to it return False
	# Else if the position has a car in it return False
	# Otherwise return the value from check_map applied to the position
	def is_valid_map_position(self, position):
		for rock in self.game.rocks:
			rock_map_position = exact_to_map(rock.position)
			
			# Avoid this cell if a rock is in it
			if position[0] == rock_map_position[0] and position[1] == rock_map_position[2]:
				return False
			
		for red_car in self.game.red_cars:
			red_car_map_position = exact_to_map(red_car.position)
				
			# Avoid this cell if a red car is in it
			if red_car_map_position[0] == position[0] and red_car_map_position[2] == position[1]:
				return False
		
		return self.game.check_map(position[0], position[1])

	# Returns True if the nearest flag can be picked up, False otherwise
	# Verifies if there is a red car on the same map position as the nearest flag, and if there is,
	# returns False as the flag can not be picked up
	def check_if_flag_can_be_picked(self):
		can_pick_flag = True
		flag_map_position = exact_to_map(self.nearest_flag.position)
		for car in self.game.red_cars:
			car_map_position = exact_to_map(car.position)
			if car_map_position[0] == flag_map_position[0] and car_map_position[2] == flag_map_position[2]:
				can_pick_flag = False
				break
		return can_pick_flag

	# Decrements the retries by one and sets the desired flag of the blue car
	def decrement_retries_and_set_desired_flag(self, position):
		self.retries -= 1
		self.desired_flag = position

	# Calculates the cost of moving through a position in the map for the A* algorithm
	# position is a map position with x and y values
	# The default cost is 1
	def position_cost(self, position):
		position_cost = 1
		for car in self.game.red_cars:
			red_car_map_position = exact_to_map(car.position)
			
			# If there is a car at a distance of 1 position in X or 1 position in Y in the map then the cost
			# Will be greater, as this is a more dangerous situation for the blue car to go through
			if (abs(red_car_map_position[0] - position[0]) == 1 and abs(red_car_map_position[2] - position[1]) == 0) \
			 or (abs(red_car_map_position[2] - position[1]) == 1 and abs(red_car_map_position[0] - position[0]) == 0):
				position_cost = 5
				break
		
		return position_cost
	
	# Applies A star algorithm from a start to a goal position
	# Returns None if either the flag can not be picked up or
	# if the time elapsed since executing the algorithm is greater than 50ms
	# Otherwise returns the shortest path if there is one
	def a_star_algorithm(self, start, goal):
		start_temp = exact_to_map(start)
		goal_temp = exact_to_map(goal.position)
		start_map_position = (start_temp[0], start_temp[2])
		goal_map_position = (goal_temp[0], goal_temp[2])
		open_set, g_dictionary, closed = self.initialize_a_star(start_map_position, goal_map_position)
		start = time.clock()
		if self.check_if_flag_can_be_picked() == False:
			return None
		while len(open_set) != 0:
			end = time.clock()
			if end - start > 0.05: # We return none if A star was running for more than 50ms
				return None
			actual = heappop(open_set)[1]
			if actual.position[0] == goal_map_position[0] and actual.position[1] == goal_map_position[1]:
				return self.path(actual)
			closed.add(actual.position)
			neighbours = self.neighbours_of(actual.position)
			for nbr in neighbours:
				position_cost = self.position_cost(nbr)
				new_g = actual.g + position_cost
				new_f = new_g + self.heuristic(nbr, goal_map_position) 
				sucessor_node = Node(new_g, new_f, nbr, actual)
					
				# If the node does not exist in the closed set,
				# Or it exists on the openSet with a higher g value
				# Then add the node to the openSet and update the g cost at the dictionary
				if nbr not in closed or new_g < g_dictionary.get(nbr, 0):
					heappush(open_set, (new_f, sucessor_node))
					g_dictionary[sucessor_node.get_position()] = new_g
		return None
	
	# Checks if there is not a nearest flag set on the player
	def check_if_has_not_nearest_flag(self):
		if self.nearest_flag is None or self.retries < 1:
			return True
		for flag in self.game.flags:
			# If this happens then the flag was not picked up yet
			if flag.position == self.nearest_flag.position:
				return False
		# If we reach here, this means that the DemoPlayer picked up the flag,
		# So it was removed from self.game.flags but we still have it set on the DemoPlayer
		return True
	
	# Store the nearest red car
	def calculate_nearest_car(self):
		self.nearest_red_car = self.get_manhattan_distances(self.game.red_cars)[0]
		return True
	
	# Check if the nearest car is at sufficient distance that we need to avoid him
	def check_nearest_car_distance(self):
		if self.distance(self.nearest_red_car) < Config.object_size[0] * 4:
			return True
		return False
	
	# Allow the player to send a smoke
	def send_smoke(self):
		if self.more_smoke == 0 and self.should_send_smoke():
			self.more_smoke = 1
		return True
	
	# Returns True if the given position is a wall on the map, false otherwise
	def is_wall(self, position):
		for wall_name in Config.wall_names:
			for symbol in Config.symbols:
				if symbol == self.game.current_map[position[1]][position[0]] \
				and Config.symbols[symbol] == wall_name:
					return True
		
		return False
	
	# Returns True if the given map position is a car
	def is_car(self, position):
		for car in self.game.red_cars:
			car_map_position = exact_to_map(car.position)
			if car_map_position[0] == position[0] and car_map_position[2] == position[1]:
				return True
		return False
	
	# Returns True if the given map position is not a car and not a rock and is a blank position
	# position is already a map position with x and y values
	...
	# Sets the direction of the blue car in order to avoid the nearest red car
	def avoid_car_helper(self):
		if self.nearest_red_car.smoked or self.nearest_red_car.immobile == 1:
			return False
		non_dir_x = sign(self.nearest_red_car.position[0] - self.position[0])
		non_dir_y = sign(self.nearest_red_car.position[1] - self.position[1])

		# Decrement the retries for this flag
		if self.nearest_flag is not None:
			self.decrement_retries_and_set_desired_flag(self.nearest_flag)

		# We are walking in a vertical motion
		if self.direction[0] == 0:
			# Evasive action, reversing vertical direction of motion.
			if non_dir_y == sign(self.direction[1]):
				self.requested_direction = (0, -non_dir_y * self.speed)
			
			elif self.nearest_red_car.direction[1] == 0 and non_dir_y == sign(self.direction[1]): # He is moving to the right or left
				self.requested_direction = (0, -non_dir_y * self.speed)

		# Horizontal motion...
		elif self.direction[1] == 0:
			# Evasive action, reversing horizontal direction of motion.
			if non_dir_x == sign(self.direction[0]):
				self.requested_direction = (-non_dir_x * self.speed, 0)
			
			elif self.nearest_red_car.direction[0] == 0 and non_dir_x == sign(self.direction[0]): # He is moving to the right or left
				self.requested_direction = (-non_dir_x * self.speed, 0)
		
		return True
	
	# Calculates the nearest flag
	def calculate_nearest_flag(self):
		distances = self.get_best_flag(self.game.flags)
		if len(distances) > 0:
			self.nearest_flag = distances[0]
			self.path_to_flag = None
		return True
	
	# Checks if there is not a path to a flag
	def check_if_has_not_path_to_flag(self):
		return self.path_to_flag is None or len(self.path_to_flag) == 0
	
	# Returns True always
	def succeed(self):
		return True
	
	# Moves the player closer to the flag if there is a path to it
	def move_to_flag(self):
		next_position = self.path_to_flag[-1]
		current_map_position = exact_to_map(self.position)

		# If we are on the same position as the next position that means we should delete that position
		# From the path and go to the position after it
		if current_map_position[0] == next_position[0] and current_map_position[2] == next_position[1]:
			del self.path_to_flag[-1]
			if len(self.path_to_flag) > 0:
				next_position = self.path_to_flag[-1]
		if len(self.path_to_flag) == 0:
			# We caught the flag, so we reset the path, the nearest and the desired flag, and the retries
			self.path_to_flag = None
			self.nearest_flag = None 
			self.reset_retries()

		if self.path_to_flag is not None:
			# Go right
			if current_map_position[0] < next_position[0] and next_position[1] == current_map_position[2]:
				self.requested_direction = (self.speed,0)
		
			# Go left
			elif current_map_position[0] > next_position[0] and next_position[1] == current_map_position[2]:
				self.requested_direction = (-self.speed,0)
		
			# Go down
			elif current_map_position[2] < next_position[1] and next_position[0] == current_map_position[0]:
				self.requested_direction = (0, self.speed)
	...
	# Helper method used to reset the number of retries to try to catch a certain flag
	# and to reset the flag that we were trying to catch
	def reset_retries(self):
		self.retries = 4
		self.desired_flag = None
	
	# Calculates the path to the nearest flag
	# If there is no path we simply go to the previous player position
	def calculate_path(self):
		self.path_to_flag = self.a_star_algorithm(self.position, self.nearest_flag)
		if self.path_to_flag is None:
			# If there is no path then decrement the retries and set the desired flag
			self.decrement_retries_and_set_desired_flag(self.nearest_flag)
			return True
		self.reset_retries()
		
		# We delete the last position in the path so that the last position always
		# contains the position to where we want to go. If we did not do this, then the last
		# position would have the position of the blue car
		del self.path_to_flag[-1]
		return len(self.path_to_flag) > 0
	
	# Check if there is a path to a flag
	def check_if_has_path_to_flag(self):
		return self.path_to_flag is not None and len(self.path_to_flag) > 0
	...
	try:
		...
		while 1:
			...
			if status == QUIT_PRESENTATION:
				...
				handler = Game(screen, info) # alterei isto para ficar como era antes
